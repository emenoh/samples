...
package de.mightypirates.megazine {
	 ...
	 * 
	 * Must be public for swf based elements (so that they can access their container page).
	 * 
	 * @author fnuecke
	 * @version 1.0.12
	 */
	internal class Page extends EventDispatcher implements IPage {
		...
		private var _elements:Array; // [even or odd][number], XML
		...
		/** Graphics library used for getting gui elements */
		private var _library:Library;
		
		/** Localizer used for localizing displayed texts */
							 ...
							 dragSpeed:Number, dragKeepDist:Number)
		{
			...
			_elements         = new Array((pageData[0] as XML).children(),
										  (pageData[1] as XML).children());
		 ...
		 */
		internal function load(even:Boolean, total:uint, thumb:Boolean = false):void {
			...
			if (!_stiff && _foldEffectAlpha[i] > 0) {
				...
				var maxAlpha:Number = Math.max(0.1, Math.min(0.9, total / 50));
				var minAlpha:Number = Math.max(0.1, maxAlpha / 4);
				...
				var coverDistFactor:Number = 0.5 + Math.abs((_pageNumber - i) / total - 0.5);
				...
				var intensityRight:Number = (_pageNumber - i) / total;
				var intensityLeft:Number = 1 - intensityRight;
				...
				if (i == 1) {
					// Odd - left page
					var alphaLeft:Number = minAlpha + (maxAlpha - minAlpha) * intensityLeft;
					foldEffectAlphas = [0, alphaLeft];
					var widthLeft:Number = minWidth + (maxWidth - minWidth) * intensityRight;
					foldEffectMatrix.createGradientBox(_pageWidth * widthLeft, _pageHeight, 0,
													   _pageWidth - _pageWidth * widthLeft);
				} else {
					// Even - right page
					var alphaRight:Number = minAlpha + (maxAlpha - minAlpha) * intensityRight;
					foldEffectAlphas = [alphaRight, 0];
					var widthRight:Number = minWidth + (maxWidth - minWidth) * intensityLeft;
					foldEffectMatrix.createGradientBox(_pageWidth * widthRight, _pageHeight);
				}
			...
			// Load the elements
			if ((_elements[i] as XMLList).length() == 0) {
				...
				_elementsLoaded[i] = 0;
				loadElement(_elements[i][(_elements[i] as XMLList).length() - 1], even);
		 ...
		 * @param even
		 */
		internal function unload(even:Boolean):void {
		...
		/**
		 * Begin loading an elements
		 * @param number The number (index) of the element to load.
		 * @param even For the even or for the odd page?
		 */
		private function loadElement(elementXML:XML, even:Boolean,
									 nocache:Boolean = false):void
		{
			
			// Element type / name
			var name:String = elementXML.name();
			
			Logger.log("MegaZine Page", "    Page " + (getNumber(even) + 1) + ": " +
			...
			if (id >= 0) {
				...
				try {
					...
					// Create the element from a class.
					var element:AbstractElement =
											new INTERNAL_CLASSES[id](_mz, _localizer, _library,
																	 this, even, elementXML);
					
					// Setup with basic stuff like position, masking etc.
					setupElement(element, even, elementXML);
					
					// Initialize it to trigger loading
					element.init();
					
				} catch (e:Error) {
					Logger.log("MegaZine Page", "    Dynamic instantiation failed.",
							   Logger.TYPE_WARNING);
					increaseElementCounter(even);
				}
				...
				// When loaded, check if everything is loaded
				loader.contentLoaderInfo.addEventListener(Event.COMPLETE,
					function(e:Event):void {
						// Setup and call the init function if it exists
						try {
							var element:AbstractElement =
									e.target.loader.getChildAt(0) as AbstractElement;
							
							element.Constructor(_mz, _localizer, _library,
												self, even, elementXML);
							...
							if (!nocache) {
								...
								loadElement(elementXML, even, true);
							} else {
								Logger.log("MegaZine Page", "    loaded element of type '"
										   + name + "' is invalid.", Logger.TYPE_WARNING);
								increaseElementCounter(even);
							}
						}
					});
				...
				loader.contentLoaderInfo.addEventListener(IOErrorEvent.IO_ERROR,
					function(e:IOErrorEvent):void {
						Logger.log("MegaZine Page", "    Page " + (_pageNumber + int(!even)) +
								  ": Failed loading element of type '" + name + "': " + e.text,
								  Logger.TYPE_WARNING);
						increaseElementCounter(even);
					});
				...
				// Begin loading
				try {
					loader.load(new URLRequest(_mz.getAbsPath("elements/" + name + ".swf"
											   + (nocache ? "?r="
											   + Math.round(Math.random() * 1000000) : ""))));
				} catch (ex:SecurityError) {
					Logger.log("MegaZine Page", "    Page " + (_pageNumber + int(!even)) +
							  ": Failed loading due to a security error.", Logger.TYPE_WARNING);
					increaseElementCounter(even);
				}
		...
		/**
		 * Setup elements by creating their mask, setting linkage if given and adding them
		 * to their container.
		 * @param element The element to setup.
		 * @param even For the even or for the odd page.
		 * @param elementXML The xml data of the object.
		 */
		private function setupElement(element:Sprite, even:Boolean, elementXML:XML):void {
			
			// Element type / name
			var name:String = elementXML.name();
			
			// Add listener to wait for loading completion
			element.addEventListener(MegaZineEvent.ELEMENT_COMPLETE, elementLoaded);
			...
			// Position / title / link
			if (elementXML.@left != undefined) {
				element.x = int(elementXML.@left.toString());
			}
			if (elementXML.@top != undefined) {
				element.y = int(elementXML.@top.toString());
			}
			...
			if (name != "img") {
				var url:String = Helper.validateString(elementXML.@url, "");
				var title:String = Helper.validateString(elementXML.@title, null);
				var titleLangs:XMLList = elementXML.elements("title");
				if (url != "") {
					url = _mz.getAbsPath(url, ["http://", "https://", "file://",
											   "ftp://", "mailto:", "anchor:"]);
					// Check if a title was not defined, if so display the link address
					if (title == null && (titleLangs == null || titleLangs.length() == 0)) {
						new ToolTip(url, element);
					}
					element.buttonMode = true;
					element.addEventListener(MouseEvent.CLICK,
				...
				// Create tooltip from title if given
				if (title != null && title != "") {
					new ToolTip(title, element);
				} else if (titleLangs != null) {
					// Generate unique id
					var uid:String = "element_" + name + "_title_" + (new Date().time).toString()
								   + int(Math.random() * 1000000).toString();
					var numLangs:uint = 0;
					for each (var lang:XML in titleLangs) {
						var langID:String = Helper.validateString(lang.@lang, "");
						if (langID != "") {
							_localizer.registerString(lang.@lang.toString(), uid,
													  lang.toString());
							numLangs++;
						}
					}
					if (numLangs > 0) {
						...
						var tt:ToolTip = new ToolTip("", element);
				...
				var hiresPath:String = Helper.validateString(elementXML.@hires, "");
				var galleryName:String = Helper.validateString(elementXML.@gallery, "");
				if (hiresPath != "" && galleryName != "") {
					hiresPath = _mz.getAbsPath(hiresPath);
					var pos:uint = _mz.getGalleryData(galleryName, getNumber(even), hiresPath);
					(element as Image).setGalleryData(galleryName, pos);
				}
			}
			
			// Add at lowest level, so we do not override foldeffects and _shadows.
			_pageContainer[int(!even)].addChildAt(element, 0);
			_pageContainer[int(!even)].addChild(mask);
		 ...
		 */
		private function elementLoaded(e:MegaZineEvent):void {
			// Check which of the pages.
			var even:Boolean = (e.page & 1) == 0;
			
			// Try to remove the event listener.
			try {
				e.target.removeEventListener(MegaZineEvent.ELEMENT_COMPLETE, elementLoaded);
			} catch (ex:Error) {}
		...
		private function increaseElementCounter(even:Boolean):void {
			...
			var i:uint = uint(!even);
			if (++_elementsLoaded[i] >= (_elements[i] as XMLList).length()) {
				loadingComplete(even);
			} else {
				setTimeout(loadElement, 100, _elements[i][(_elements[i] as XMLList).length()
											 - 1 - _elementsLoaded[i]], even);
			}
