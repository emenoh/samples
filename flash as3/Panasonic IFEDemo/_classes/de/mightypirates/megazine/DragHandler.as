...
package de.mightypirates.megazine {
	 ...
	 * dragging or turning, and for making _pages turn.
	...
	internal class DragHandler extends EventDispatcher {
		...
		/** The image to use as the "turn left" cursor */
		private var _curTurnLeft:DisplayObject;
		
		/** The image to use as the "turn right" cursor */
		private var _curTurnRight:DisplayObject;
		...
		private function mouseLeave(e:Event):void {
			...
			if (_draggedPage == null || _draggedPage.state != Page.DRAGGING_USER) {
				if ((_curTurnLeft != null && _curTurnLeft.visible)
					|| (_curTurnRight != null && _curTurnRight.visible)) {
					_curTurnLeft.visible = false;
					_curTurnRight.visible = false;
					Mouse.show();
				}
				cancelDrag();
			}
		...
		private function mouseMove(e:MouseEvent = null):void {
			...
			// Position cursor images
			if (_curTurnLeft != null && _curTurnLeft.parent != null) {
				_curTurnLeft.x = _curTurnLeft.parent.mouseX;
				_curTurnLeft.y = _curTurnLeft.parent.mouseY;
			}
			if (_curTurnRight != null && _curTurnRight.parent != null) {
				_curTurnRight.x = _curTurnRight.parent.mouseX;
				_curTurnRight.y = _curTurnRight.parent.mouseY;
			}
										...
										_draggedPage.state == Page.DRAGGING_USER))
			{
				...
				// OK, it's a corner, begin dragging (only for non stiff _pages, though).
			...
			if (_pagesOdd.visible && _pagesEven.visible) {
				if (_pages[_currentPage - 1] && (curPos & CURSOR_LEFT)) {
					if (_handCursor || _curTurnLeft == null) {
						_mz.buttonMode = true;
					} else if (!_curTurnRight.visible) {
						Mouse.hide();
						_curTurnLeft.visible = true;
					}
				} else if (_pages[_currentPage] && (curPos & CURSOR_RIGHT)) {
					if (_handCursor || _curTurnLeft == null) {
						_mz.buttonMode = true;
					} else if (!_curTurnLeft.visible) {
						Mouse.hide();
						_curTurnRight.visible = true;
					}
		 ...
		 * @param _leftToRight Only used if not currently dragging a page. When used specifies in
		 * which direction to turn.
		 * @param count How many _pages to turn
		 * @param delay The delay in milliseconds between the _pages start turning (only used if
		 * count > 1)
