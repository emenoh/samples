...
package de.mightypirates.megazine {
	...
	internal class Chapter {
		...
		public function Chapter(mz:MegaZine, chapterXML:XML, number:int,
			...
			// Background sound
			if (chapterXML.@bgsound != undefined && chapterXML.@bgsound.toString() != "") {
				
				var url:String = mz.getAbsPath(chapterXML.@bgsound.toString());
		...
		private function onTimer(e:TimerEvent = null):void {
			if (_muted) {
				...
				|| Math.abs(_soundTrans.volume - _volTarget) < 2 * _increment)
			{
				_soundTrans.volume = _volTarget;
				_volTimer.stop();
			} else {
				_soundTrans.volume += _soundTrans.volume < _volTarget ? _increment : -_increment;
			}
